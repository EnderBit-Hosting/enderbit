<?php
$title = "Games We Support";
$css = "games";
include 'includes/head.php';
include 'includes/nav.php';
?>

<section class="page-hero text-center">
  <div class="container" data-animate>
    <h1>Every Server Type. All Free.</h1>
    <p class="subheadline">From Minecraft to MariaDB — if you need a server, EnderBit has you covered.</p>
  </div>
</section>

<section class="games-grid-section container">
  <div class="games-grid">
    <!-- Game 1 -->
    <div class="game-card" data-animate style="transition-delay: 0ms;">
      <img src="https://www.minecraft.net/content/dam/games/minecraft/key-art/Games_Subnav_Minecraft-300x465.jpg" alt="Minecraft" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">Minecraft</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 2 -->
    <div class="game-card" data-animate style="transition-delay: 50ms;">
      <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/730/capsule_616x353.jpg" alt="CS2" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">CS2</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 3 -->
    <div class="game-card" data-animate style="transition-delay: 100ms;">
      <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/252490/capsule_616x353.jpg" alt="Rust" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">Rust</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 4 -->
    <div class="game-card" data-animate style="transition-delay: 150ms;">
      <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/346110/capsule_616x353.jpg" alt="ARK" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">ARK: Survival Evolved</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 5 -->
    <div class="game-card" data-animate style="transition-delay: 200ms;">
      <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/4000/capsule_616x353.jpg" alt="Garry's Mod" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">Garry's Mod</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 6 -->
    <div class="game-card" data-animate style="transition-delay: 250ms;">
      <img src="https://cdn.cloudflare.steamstatic.com/steam/apps/581320/capsule_616x353.jpg" alt="Insurgency" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Game Server</div>
      <div class="game-content">
        <div class="game-name">Insurgency: Sandstorm</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 7 -->
    <div class="game-card" data-animate style="transition-delay: 300ms;">
      <img src="https://assets-global.website-files.com/6257adef93867e50d84d30e2/636e0a6a49cf127bf92de1e2_icon_clyde_blurple_RGB.png" alt="Discord Bots" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Bot Hosting</div>
      <div class="game-content">
        <div class="game-name">Discord Bots</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 8 -->
    <div class="game-card" data-animate style="transition-delay: 350ms;">
      <img src="https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/teamspeak.png" alt="Mumble / TeamSpeak" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Voice Server</div>
      <div class="game-content">
        <div class="game-name">Mumble / TeamSpeak</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 9 -->
    <div class="game-card" data-animate style="transition-delay: 400ms;">
      <img src="https://mariadb.com/wp-content/uploads/2019/11/mariadb-logo-vert_blue-transparent.png" alt="MariaDB" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Database</div>
      <div class="game-content">
        <div class="game-name">MariaDB</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
    <!-- Game 10 -->
    <div class="game-card" data-animate style="transition-delay: 450ms;">
      <img src="https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/mongodb.png" alt="MongoDB" loading="lazy">
      <div class="game-overlay"></div>
      <div class="badge-category">Database</div>
      <div class="game-content">
        <div class="game-name">MongoDB</div>
        <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="host-link">Host for Free &rarr;</a>
      </div>
    </div>
  </div>
</section>

<section class="features-accordion container">
  <div class="accordion" data-animate>
    <div class="accordion-item">
      <button class="accordion-header">
        <h3>Game Servers</h3>
        <svg class="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
      </button>
      <div class="accordion-content">
        <div class="accordion-inner">
          <p>What makes EnderBit game servers different. Mods support, custom startup flags, automatic restarts. Full FTP access and a powerful web-based file manager. Schedule tasks and manage backups directly from the dashboard.</p>
        </div>
      </div>
    </div>
    <div class="accordion-item">
      <button class="accordion-header">
        <h3>Bot Hosting</h3>
        <svg class="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
      </button>
      <div class="accordion-content">
        <div class="accordion-inner">
          <p>24/7 uptime for Discord bots, Node.js and Python support, persistent storage. Keep your community engaged without worrying about your bot going offline when you turn off your PC.</p>
        </div>
      </div>
    </div>
    <div class="accordion-item">
      <button class="accordion-header">
        <h3>Database Hosting</h3>
        <svg class="chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
      </button>
      <div class="accordion-content">
        <div class="accordion-inner">
          <p>MariaDB and MongoDB instances, persistent data, accessible from your other EnderBit services. Perfect for storing player stats, economy data, or bot configurations securely.</p>
        </div>
      </div>
    </div>
  </div>
</section>

<section class="final-cta" data-animate>
  <div class="container">
    <h2>Pick your game. Start your server.</h2>
    <p>It takes less than 2 minutes. No payment needed. Ever.</p>
    <a href="https://dash.enderbit.com" target="_blank" rel="noopener noreferrer" class="btn-primary btn-large">Create Your Free Server</a>
  </div>
</section>

<script>
document.addEventListener('DOMContentLoaded', () => {
  // Make game card images clickable
  document.querySelectorAll('.game-card img').forEach(img => {
    img.style.cursor = 'pointer';
    img.addEventListener('click', () => {
      window.open('https://dash.enderbit.com', '_blank', 'noopener,noreferrer');
    });
  });

  const headers = document.querySelectorAll('.accordion-header');
  headers.forEach(header => {
    header.addEventListener('click', () => {
      const item = header.parentElement;
      const isActive = item.classList.contains('active');
      
      // Close all
      document.querySelectorAll('.accordion-item').forEach(i => {
        i.classList.remove('active');
        i.querySelector('.accordion-content').style.maxHeight = null;
      });
      
      // Open clicked if it wasn't active
      if (!isActive) {
        item.classList.add('active');
        const content = item.querySelector('.accordion-content');
        content.style.maxHeight = content.scrollHeight + "px";
      }
    });
  });
});
</script>

<?php include 'includes/footer.php'; ?>